...
public class FreeboardDao {
	...
	//getList
		public List<Freeboard> getList(Map<String, Integer> map){   
			...
			list = mapper.selectList("freeboard.getList",map);
			return list;
		}
		...
		public Freeboard getOne(int idx) {
			SqlSession mapper = factory.openSession();
			Freeboard dto = mapper.selectOne("freeboard.selectByIdx", idx);  
			mapper.close();
			return dto;
		}
		...
		public  void update(Freeboard dto) {
			SqlSession mapper = factory.openSession();
			mapper.insert("freeboard.update", dto);
			mapper.commit();
			mapper.close();
		...
		public Freeboard passwordCheck(Map<String, Object> map) {
			SqlSession mapper = factory.openSession();
			Freeboard dto = mapper.selectOne("freeboard.passwordCheck",map);
			mapper.close();
			return dto;
